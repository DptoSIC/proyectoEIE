...
public class proyectoDAOMock implements proyectoDAO {
	...
	private List<Proyecto> listaProyecto = generarProyectos();
	...
	public List<Proyecto> filtrarPorCodigo(int i) {
		List<Proyecto> listaFiltrada = new ArrayList<>();
		for (Proyecto p:listaProyecto) {
			if (p.getIdProyecto()==i) {
				listaFiltrada.add(p);
			}
		}
	...
	public List<Proyecto> filtrarPorTitulo(String titulo) {
		List<Proyecto> listaFiltrada = new ArrayList<>();
		for (Proyecto p:listaProyecto) {
	...
	public List<Proyecto> filtrarPorDescripcion(String desc) {
		List<Proyecto> listaFiltrada = new ArrayList<>();
		for (Proyecto p:listaProyecto) {
			if (p.getDescripcionCorta().equals(desc)) {
				listaFiltrada.add(p);
			}
		}
		return listaFiltrada;
	...
	public List<Proyecto> filtrarPorCurso(String curso) {
		List<Proyecto> listaFiltrada = new ArrayList<>();
		for (Proyecto p:listaProyecto) {
			if (p.getCurso().toString().equals(curso)) {
				listaFiltrada.add(p);
			}
		}
		return listaFiltrada;
	...
	public List<Proyecto> filtrarPorEstado(Estado estado) {
		List<Proyecto> listaFiltrada = new ArrayList<>();
		for (Proyecto p:listaProyecto) {
